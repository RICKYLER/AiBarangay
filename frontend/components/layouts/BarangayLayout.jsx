...
import { Link, NavLink, useLocation, useNavigate } from '@/lib/router-shim';
import {
...
import { ORG } from '@/lib/data/adminData';
import { useAuth } from '@/hooks/useAuth';
import { useNotifications } from '@/hooks/useNotifications';

const PAGE_META = {
  '/admin/dashboard': { title: 'Barangay Operations Dashboard', category: 'Operations' },
  '/admin/messages': { title: 'Messages — Desk Inbox', category: 'Operations' },
  ...
  '/admin/field-operations': { title: 'Field Operations', category: 'Operations' },
  '/admin/ai-intelligence': { title: 'AI Intelligence', category: 'Operations' },
...
function currentMeta(pathname) {
  if (/^\/admin\/reports\/[^/]+\/review/.test(pathname)) {
...
const OPERATIONS_NAV = [
  ...
  { to: '/admin/field-operations', label: 'Field Operations', icon: Radio },
  { to: '/admin/ai-intelligence', label: 'AI Intelligence', icon: Cpu },
...
export default function BarangayLayout({ children }) {
  ...
  const isDeskOfficer = user?.role === 'DESK_OFFICER';

  /* Auth gate — staff portal; residents are bounced to their own
     portal. DESK_OFFICER accounts are chat-only: the nav below
     shrinks to Messages and any other /admin page redirects there. */
  useEffect(() => {
    if (loading) return;
    if (!user) navigate('/login');
    else if (user.role === 'RESIDENT') navigate('/resident/dashboard');
    else if (user.role === 'DESK_OFFICER' && location.pathname !== '/admin/messages') {
      navigate('/admin/messages');
    }
  }, [loading, user, navigate, location.pathname]);

  /* Adapt API notifications to the panel's { text, time, unread } shape. */
  const notifItems = (notifications || []).slice(0, 8).map((n) => ({
    id: n.id,
    text: n.title,
  ...
  /* Sign out server-side first (revokes the session + clears the
     cookie) — navigating alone would leave the 14-day cookie alive. */
            ...
            {isDeskOfficer ? (
              <>
                <div className="gov-nav-section">
                  <span className="gov-nav-section-label">MESSAGES</span>
                </div>
                <NavItem
                  item={{ to: '/admin/messages', label: 'Messages', icon: MessageSquare }}
                ...
                {userMenuOpen && (
                  <div className="gov-dropdown">
                    <div className="gov-dropdown-head">
                      ...
                      <div className="gov-meta">{ORG.barangay} · {user ? user.role.replace('_', ' ') : ORG.role}</div>
                    </div>
