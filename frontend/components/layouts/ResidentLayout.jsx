...
export default function ResidentLayout({ children }) {
  ...
  /* Auth gate — the resident portal is private. */
  useEffect(() => {
    if (!loading && !user) navigate('/login');
  }, [loading, user, navigate]);

  /* Live badge on the notifications nav item. */
  const navGroups = NAV_GROUPS.map((group) => ({
    ...group,
    links: group.links.map((link) =>
      link.to === '/resident/notifications' ? { ...link, badge: unread } : link
    ),
  }));
        ...
        <aside ref={asideRef} className={sideClass}>
          ...
          <div className="res-side-footer">
            ...
            <Link to="/resident/profile" className="res-side-account" onClick={closeDrawer}>
              <span className="res-side-avatar" aria-hidden="true">{user?.firstName?.[0] || '?'}</span>
              <span className="res-side-account-text">
                <span className="res-side-account-name">{user ? `${user.firstName} ${user.lastName}` : '…'}</span>
            ...
            <div className="res-header-right">
              ...
              <Link to="/resident/profile" className="res-header-profile">
                ...
                <span className="res-header-profile-text">
                  <span className="res-header-profile-name">{user?.firstName || '…'}</span>
