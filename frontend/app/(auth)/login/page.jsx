...
export default function LoginPage() {
  ...
  /* Already signed in (e.g. a new tab landed here) — the session cookie
     is still valid, so skip the form and go straight to the portal. */
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      /* Both tabs hit the same account; the role decides the portal,
         so a resident on the Personnel tab (or staff on the Resident
         tab) is simply taken to their own dashboard. */
      const staff = !['RESIDENT'].includes(user.role);
      setNotice({
        type: 'success',
        text: staff
          ? 'Personnel account detected — opening the operations portal…'
          : `Welcome back, ${user.firstName}! Opening your resident dashboard…`,
      });
      setTimeout(() => navigate(homeForRole(user.role)), 900);
    } catch (err) {
      setErrors({});
      setNotice({ type: 'error', text: err.message || 'Sign-in failed. Please try again.' });
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <div className="pub-auth-card pub-fade">
            ...
            <form className="pub-auth-body-form" onSubmit={handleSubmit} noValidate>
              ...
              <div className="pub-row-between pub-wrap">
                ...
                <Link to="/forgot-password" className="pub-link" style={{ fontSize: 13 }}>Forgot Password?</Link>
              </div>
              ...
              <div className="pub-auth-alert" style={{ background: 'var(--pub-surface-inset)', border: '1px solid var(--pub-border)', color: 'var(--pub-text-2)' }}>
                ...
                <span>
                  {role === 'resident'
                    ...
                    : 'Personnel accounts are provisioned by the LGU. Demo: pedro.ramos@example.com / Password123!'}
                </span>
              </div>
            </form>
