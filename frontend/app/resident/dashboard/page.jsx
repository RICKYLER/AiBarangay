...
import { toCardReport } from '@/lib/reportAdapter';
 ...
 * Report counts come from the resident's own reports (RLS-enforced).
 */
export default function ResidentDashboard() {
  ...
  const summary = useMemo(() => ({
    total: cards.length,
    underReview: cards.filter((r) => r.status === 'Submitted' || r.status === 'Under Review').length,
    inProgress: cards.filter((r) => ['Verified', 'Assigned', 'Field Response', 'Reopened'].includes(r.status)).length,
    resolved: cards.filter((r) => r.status === 'Resolved' || r.status === 'Closed').length,
  }), [cards]);
      ...
      <section className="res-greet" aria-labelledby="res-greet-title">
        <div>
          ...
          <h1 id="res-greet-title" className="res-greet-title">
            {greeting}, {user?.firstName || 'Neighbor'}
          </h1>
      ...
      <section className="res-section" aria-labelledby="res-summary-title">
        <div className="res-section-head">
          ...
          <span className="res-demo-note">
            {loading ? 'Loading…' : error ? 'Could not load reports' : `${summary.total} submitted`}
          </span>
        </div>
        ...
        <div className="res-stats">
          <div className="res-stat">
            ...
            <span className="res-stat-value">{loading ? '—' : summary.total}</span>
          ...
          <div className="res-stat warning">
            ...
            <span className="res-stat-value">{loading ? '—' : summary.underReview}</span>
          ...
          <div className="res-stat accent">
            ...
            <span className="res-stat-value">{loading ? '—' : summary.inProgress}</span>
          ...
          <div className="res-stat success">
            ...
            <span className="res-stat-value">{loading ? '—' : summary.resolved}</span>
          ...
          {recent.length > 0 ? (
            ...
            <div className="res-empty">
              <p className="res-empty-title">
                {loading ? 'Loading your reports…' : 'No reports yet'}
              </p>
              <p className="res-empty-text">
                When you submit a report, its progress will appear here.
              </p>
