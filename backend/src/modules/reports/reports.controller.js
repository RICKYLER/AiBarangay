...
export const createReport = userRoute(async (req, res, client, session) => {
  ...
  // ---- 4. Media (photo uploaded via multer) --------------------------
  if (req.file) {
    const publicUrl = `${process.env.BACKEND_ORIGIN || 'http://localhost:4000'}/uploads/${req.file.filename}`;
    await client.query(
      ...
      [
        reportRow.id, session.id, publicUrl,
        req.file.mimetype, req.file.size, path.join('uploads', req.file.filename),
